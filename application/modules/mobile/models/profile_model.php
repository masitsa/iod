...
class Profile_model extends CI_Model 
{
	...
	*/
	...
	public function create_file_name($client_id, $receiver_id, $web_name = NULL)
	{
		...
		$client_query = $this->db->get('jos_users');
		$sender_web_name = '';
		
		if($client_query->num_rows() > 0)
		{
			$client_row = $client_query->row();
			$sender_web_name = $this->create_web_name($client_row->username);
		}
		
		//get receiver web name
		$this->db->select('username');
		$this->db->where('id', $receiver_id);
		$receiver_query = $this->db->get('jos_users');
		$receiver_web_name = '';
		
		if($receiver_query->num_rows() > 0)
		{
			$receiver_row = $receiver_query->row();
			$receiver_web_name = $this->create_web_name($receiver_row->username);
		}
		...
		if(empty($file_name))
		{
			...
			if($query->num_rows() > 0)
			{
				...
				$update_data['last_receiver_web_name'] = $receiver_web_name;
			...
			else
			{
				...
				$data['last_receiver_web_name'] = $receiver_web_name;
