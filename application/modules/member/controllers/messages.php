...
class Messages extends member 
{
	...
	{
		//browse all profiles
		$where = 'message.member_id = '.$this->member_id.' OR message.receiver_id = '.$this->member_id;
		$table = 'message';
		$limit = NULL;
		...
		$v_data['current_member_id'] = $this->member_id;
	...
	public function view_message($receiver_web_name)
	{
		...
		$v_data['sender'] = $this->profile_model->get_client($this->member_id);
		$v_data['messages'] = $this->profile_model->get_messages($this->member_id, $receiver_id, $this->messages_path);
	...
	{
		$data['unread_messages'] = $this->messages_model->count_unread_messages($this->member_id, $this->messages_path);
